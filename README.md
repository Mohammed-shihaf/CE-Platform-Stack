# CE-Platform-Stack — branch CE-052
...
|---|---|---|
| Webpack (`@angular-devkit/build-angular:browser`, the legacy builder) | pnpm | Modular Monolith |
...
identical to branch `CE-A6` (same bundler + package manager
...
`CE-A6`. It is not re-run in full on every one of the 60
matrix branches for sandbox time reasons — this branch's only diff
from `CE-A6` is the architecture-note text in this README, so
