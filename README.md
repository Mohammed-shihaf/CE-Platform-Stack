# CE-Platform-Stack — branch CE-050
...
|---|---|---|
| Webpack (`@angular-devkit/build-angular:browser`, the legacy builder) | yarn (Berry, `yarn set version berry`) | Distributed System |
...
identical to branch `CE-046` (same bundler + package manager
...
`CE-046`. It is not re-run in full on every one of the 60
matrix branches for sandbox time reasons — this branch's only diff
from `CE-046` is the architecture-note text in this README, so
