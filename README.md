# CE-Platform-Stack — branch CE-035
...
|---|---|---|
| Vite (not a real standalone Angular CLI production bundler in Angular 20 — esbuild used instead; see "About the 'Vite' requirement" below) | pnpm | Distributed System |
...
identical to branch `CE-A3` (same bundler + package manager
...
(`pnpm start` in each backend package, `pnpm exec ng serve`
...
`CE-A3`. It is not re-run in full on every one of the 60
matrix branches for sandbox time reasons — this branch's only diff
from `CE-A3` is the architecture-note text in this README, so
