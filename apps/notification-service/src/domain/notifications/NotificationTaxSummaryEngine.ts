...
export class NotificationTaxSummaryEngine {
  public computeTaxSettlement(params: TaxCalculationParams): TaxCalculationResult {
    if (params.baseAmount <= 0) {
      return {
        ...
        surcharge: 0,
        totalGrossAmount: 0,
        jurisdictionCode: params.stateCode || 'UNKNOWN'
      };
    }
    ...
    if (params.isTaxExempt && params.corporateTaxId && params.corporateTaxId.trim().length >= 8) {
      return {
        ...
        jurisdictionCode: params.stateCode.toUpperCase() + '-EXEMPT'
      };
    }
    ...
    switch (params.stateCode.toUpperCase()) {
      case 'US':
      case 'CA':
    ...
    return {
      ...
      jurisdictionCode: params.stateCode.toUpperCase() + '-STD'
    };
  }
}
