...
BRANCHES = [
    ...
    ("CE-PYJS-002", "setuptools", "npm", "Microservices"),
    ("CE-PYJS-003", "poetry", "pnpm", "Monolith"),
    ("CE-PYJS-004", "poetry", "pnpm", "Microservices"),
    ("CE-PYJS-005", "hatch", "yarn (Berry)", "Monolith"),
    ("CE-PYJS-006", "hatch", "yarn (Berry)", "Microservices"),
    ("CE-PYJS-007", "flit", "bun", "Monolith"),
    ("CE-PYJS-008", "flit", "bun", "Microservices"),
    ("CE-PYJS-009", "uv", "npm (esbuild/vite)", "Monolith"),
    ("CE-PYJS-010", "uv", "npm (esbuild/vite)", "Microservices"),
]
...
|---|---|---|---|
| `{branch_name}` | {py_tool} | {js_pkg} | **{arch}** |
...
def main():
    ...
    run_cmd("git checkout CE-PYTS-001", repo_dir)

    for branch_name, py_tool, js_pkg, arch in BRANCHES:
        print(f"--> Creating & Pushing branch {branch_name} ({py_tool} / {js_pkg} / {arch})...")
        ...
        run_cmd(f'git commit -m "{branch_name}: Python ({py_tool}) + JS ({js_pkg}) - {arch}"', repo_dir)
